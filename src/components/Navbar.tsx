  ...
  return (
    <header className="sticky top-3 z-50 w-full px-4 flex items-center justify-center">
      <div className="w-full max-w-4xl mx-auto flex items-center justify-between gap-2 px-3 py-2 rounded-full border border-zinc-300/80 dark:border-zinc-800/80 bg-background/85 backdrop-blur-xl shadow-xl shadow-black/5 dark:shadow-black/40 transition-colors duration-200">
        ...
        <nav className="hidden md:flex items-center gap-1 px-3 py-1 rounded-full border border-zinc-300/60 dark:border-zinc-800/60 bg-zinc-200/50 dark:bg-zinc-900/60 shadow-inner">
          {dockLinks.map((item) => {
              ...
              >
                ...
                {isActive && (
                  <motion.span
                    ...
                    className="absolute -bottom-1 left-1/2 -translate-x-1/2 size-1 rounded-full bg-emerald-500 shadow-[0_0_6px_#10b981]"
                    transition={{ type: "spring", stiffness: 350, damping: 30 }}
                  />
                )}

                {/* macOS Hover Tooltip */}
                <span className="pointer-events-none absolute -bottom-9 left-1/2 -translate-x-1/2 px-2 py-0.5 rounded-md bg-zinc-900 text-white dark:bg-zinc-100 dark:text-zinc-900 text-[10px] font-mono font-medium opacity-0 group-hover:opacity-100 group-hover:-translate-y-0.5 transition-all duration-200 shadow-md whitespace-nowrap z-50">
                  {item.name}
                </span>
              </motion.a>
            );
        ...
        <div className="flex items-center gap-2 pr-1">
          {mounted && (
            <motion.button
              ...
              className="p-2 rounded-full text-muted-foreground hover:text-foreground hover:bg-zinc-200/80 dark:hover:bg-zinc-800/80 transition-all border border-transparent hover:border-zinc-300 dark:hover:border-zinc-800 cursor-pointer"
            >
          ...
          <button
            ...
            className="md:hidden p-2 rounded-full text-muted-foreground hover:text-foreground hover:bg-zinc-200/80 dark:hover:bg-zinc-800/80 transition-all cursor-pointer"
          >
